...
//   ㅡ> 값이나 특정 대상을 저장하기 위한 변수 생성을 위해 사용

//1. 원시형(PrimitiveType)/기본형 : 값을 표현하기 위한 자료형 - 키워드
//  숫자형(정수형, 실수형, 문자형)- 연산 가능 , 논리형

//2. 참조형(ReferenceType) : 특정 대상을 표현하기 위한 자료형 - 식별자(클래스)
//  String 클래스 : 문자열을 표현하기 위한 자료형(클래스)

//컴퓨터에서 값을 표현하는 단위 : 1Bit(0 또는 1) , 1Byte(8Bit) : Character (영문자)
// * 1024Byte = 1KByte / 1024KByte = 1MByte / 1024Myte = 1GByte / 1024GByte = 1TByte
// * 1Gbps (Bit per Second) : 초당 전송하는 Gbit / 1Gcps (character per Second) : 초당 전송하는 문자수
//1Byte >> 00110010 (50) >> 11001101 (1의 보수화) >> 11001110(2's) -50 : 11001110
//: 8bit/ 양수 음수를 구분하기 위해 앞에 0,1 양수가 0
// 50(10진수) = 110010(2진수) / -50(10) = -110010(2) 
...
public class DataType {
	public static void main(String[] args) {
		...
		// => Java의 정수형 리터럴은 기본적으로 4Byte로 표현 (-2147483648~2147483647)
		System.out.println("<<정수형(IntegerType)>>");
		System.out.println("정수값(10진수100)="+100); //숫자값 >> 10진수 정수값을 표현한 리터럴
		//println 메소드는 무조건 10진수의 정수값으로 변환하여 출력 처리
		System.out.println("정수값(8진수100)="+0100); //0숫자값 >> 8진수 정수값을 표현한 리터럴
		System.out.println("정수값(16진수100)="+0x100); //0x숫자값 >> 16진수 정수값을 표현한 리터럴
		System.out.println("정수값(10진수100)="+100L); //숫자값L >> 8Byte(Long)의 정수값을 표현한 리터럴
		// ex) 2147483648값은 4byte가 넘어가므로 2147483648L로 표현해야 함
		
		System.out.println("정수값="+2147483647); //4Byte로 표현 가능한 정수값 = -2147483648~2147483647
		// => 4Byte로 표현 불가능한 정수값을 사용할 경우 에러 발생
		//System.out.println("점수값="+2147483648);
		//4Byte로 표현 불가능한 정수값은 정수값 뒤에 [L]을 붙여 8Byte 정수값으로 표현하여 사용
